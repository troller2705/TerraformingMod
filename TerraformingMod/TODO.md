...
- [ ] **Global Rename:** Change all instances of `GasType.Volatiles` to `GasType.Methane`.
- [ ] **Gas Arrays:** Update any hardcoded gas array limits to use `(int)Chemistry.GasType.Count` to support the new 32-gas system.
- [ ] **GasMixtures:** Refactor global tick logic to use native `GasMixture.Add()` / `GasMixture.Remove()` directly instead of instantiating new objects.
